...
export interface IGitPanelState {
  ...
  /**
   * GitLab auth status — null means extension not installed or status unknown.
...
export class GitPanel extends React.Component<IGitPanelProps, IGitPanelState> {
  ...
  componentDidMount(): void {
    ...
    // Poll GitLab auth status so the connection banner stays current
    this._refreshGitLabStatus();
    this._gitLabPollId = window.setInterval(
      () => this._refreshGitLabStatus(),
      30_000
    );
  ...
  private _renderGitLabBanner(): React.ReactElement | null {
    const { gitLabStatus } = this.state;
    // null means extension definitively not installed (404) — hide entirely
    ...
    const initial = (
      gitLabStatus.display_name ?? gitLabStatus.username ?? '?'
    )
      .trim()
      .charAt(0)
      .toUpperCase();
        ...
        >
          ...
          >
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              {gitLabStatus.username && gitLabStatus.display_name && (
                <div
                  style={{
          ...
          >
            ...
            <span
              ...
              onClick={() => void commands.execute('gitlab-auth:disconnect')}
              onKeyDown={e =>
                e.key === 'Enter' &&
                void commands.execute('gitlab-auth:disconnect')
              }
            >
              Disconnect
            </span>
      ...
      >
        ...
        >
          ...
          <svg
            ...
            fill="currentColor"
          >
            <path d="M8 .5L5.5 6h5L8 .5zM2 6L.5 10.5 8 15.5l7.5-5L14 6l-3 4.5H5L2 6z" />
          </svg>
        ...
        >
          Connect to GitLab so commits are attributed to your identity
          automatically, and push/pull authenticate without a password.
        </p>
        <button
          onClick={() => void commands.execute('gitlab-auth:connect')}
          style={{
            width: '100%',
            padding: '6px 0',
            background: 'var(--jp-brand-color1, #1976d2)',
  ...
  /**
   * Resolves the git commit author string for the current commit.
   *
   * Priority:
   *   1. Active GitLab session (from jupyter-server-gitlab-auth) — identity is
   *      determined from the Keycloak/oauth2-proxy session, never stored in git config.
   *   2. Fallback to the upstream "Who is committing?" dialog for non-GitLab setups.
   *
   * The returned string is passed to `git commit --author "Name <email>"` so it is
   * applied only to this specific commit and never written to git config.
   *
  ...
  private async _hasIdentity(path: string | null): Promise<string | null> {
    ...
    // Identity comes from this browser session's cached state (refreshed every 30s).
    // We never re-fetch from the server here — the committer is whoever is logged in
    // in THIS browser tab, not whoever the server last saw.
    const status = this.state.gitLabStatus;

    if (status !== null) {
      // Auth extension is installed
      if (status.connected && status.display_name && status.email) {
        return `${status.display_name} <${status.email}>`;
      }

      // Extension present but not connected — prompt to connect
      const result = await showDialog({
        title: this.props.trans.__('Connect to GitLab to commit'),
        body: (
          <div style={{ padding: '8px 0' }}>
            <p style={{ margin: '0 0 8px' }}>
              {this.props.trans.__(
                'Your GitLab session is not connected. ' +
                  'Connect to GitLab so your commits are attributed to the correct identity.'
              )}
            </p>
            <p style={{ margin: 0, fontSize: 12, color: 'var(--jp-ui-font-color2)' }}>
              {this.props.trans.__(
                'Click "Connect" to start the authorization flow, then retry the commit.'
              )}
            </p>
          </div>
        ) as any,
        buttons: [
          ...
          Dialog.okButton({ label: this.props.trans.__('Connect to GitLab') })
        ]
      });

      if (result.button.accept) {
        await this.props.commands.execute('gitlab-auth:connect');
        // Force a fresh fetch after connecting and use the returned value directly
        // (setState is async so this.state may not reflect the new status yet)
        const newStatus = await this._refreshGitLabStatus();
        if (newStatus?.connected && newStatus.display_name && newStatus.email) {
          return `${newStatus.display_name} <${newStatus.email}>`;
        }
      }

      throw new Error(
        this.props.trans.__('GitLab not connected. Connect to GitLab first.')
      );
    }

    // --- Fallback: upstream identity dialog (GitLab extension not installed) ---
    return this._hasIdentityFallback(path);
  }

  /**
   * Original upstream identity resolution — used as fallback when the GitLab
   * auth extension is not installed. Reads/writes git config and shows a dialog.
   */
  private async _hasIdentityFallback(path: string | null): Promise<string | null> {
    if (!path) {
      return null;
    }
