...
class GitHandler(APIHandler):
    ...
    async def _get_gitlab_auth(self):
        """
        Returns {"username": ..., "password": <access_token>} for the user who
        made this HTTP request, by looking up the shared token store that
        jupyter-server-oauth-providers registers in self.settings.

        Identity comes from the same header resolution order used by the auth
        extension: kubeflow-userid → X-Auth-Request-User → X-Forwarded-User.

        Returns None if the auth extension is not installed, the user is not
        connected, or no identity header is present.
        """
        token_store = self.settings.get("oauth_token_store")
        if not token_store:
            return None
        ...
        for provider_name in ("gitlab", "github"):
            token_key = f"{provider_name}:{user_id}"
            token_data = await token_store.load_token(token_key)
            if token_data and token_data.get("access_token"):
                return {
                    "username": token_data.get("provider_username", "oauth2"),
...
class GitCloneHandler(GitHandler):
    ...
    async def post(self, path: str = ""):
        ...
        response = await self.git.clone(
            ...
            data["clone_url"],
            data.get("auth") or await self._get_gitlab_auth(),
...
class GitPushHandler(GitHandler):
    ...
    async def post(self, path: str = ""):
        ...
        # Resolve auth: explicit from request body takes priority, then GitLab session
        auth = data.get("auth") or await self._get_gitlab_auth()
