...
class Git:
    ...
    async def commit(self, commit_msg, amend, path, author=None):
        ...
        if author:
            ...
            # git needs a committer identity separate from --author.
            # Parse "Name <email>" and inject via env vars — no git config writes.
            m = re.match(r"^(.+?)\s*<([^>]+)>", author)
            if m:
                env["GIT_COMMITTER_NAME"] = m.group(1).strip()
                env["GIT_COMMITTER_EMAIL"] = m.group(2).strip()
        if amend:
            cmd.extend(["--amend", "--no-edit"])
        else:
            cmd.extend(["--m", commit_msg])

        code, _, error = await self.__execute(cmd, cwd=path, env=env)
